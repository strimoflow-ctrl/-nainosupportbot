...
def handle_user_msg(message):
    ...
    # User ke message ko history aur GROUP ke ussi topic mein save karna (Cleanest format)
    save_msg(user_id, 'user', text)
    bot.send_message(GROUP_ID, f"{text}", message_thread_id=topic_id) # 💬 emoji bhi hata diya
